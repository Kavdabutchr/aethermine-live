    ...
    def do_GET(self):
        ...
        if path == '/' or path == '/admin':
            ...
            try:
                ...
                try:
                    cur.execute("""
                        SELECT user_id, username, first_name, plan, 
                               daily_earn as daily, joined_at,
                               CASE 
                                   WHEN plan = 'free' THEN 0
                                   WHEN plan IN ('trial', 'starter') THEN 0.05
                                   WHEN plan IN ('bronze', 'silver') THEN 0.10
                                   WHEN plan IN ('gold', 'platinum') THEN 0.15
                                   WHEN plan IN ('supreme', 'diamond') THEN 0.20
                                   ELSE 0
                    ...
                    for ref in referrals:
                        ref_dict = dict(ref)
                        if ref_dict.get('joined_at'):
                            ref_dict['joined_at'] = ref_dict['joined_at'].isoformat()
                        # Convert Decimal to float for JSON serialization
                        ref_dict['commission_rate'] = float(ref_dict.get('commission_rate', 0))
                        ref_dict['daily'] = float(ref_dict.get('daily', 0))
                ...
                logger.exception("api/referrals error")
                self._respond(500, 'application/json', b'{"error":"Internal error"}')
        else:
